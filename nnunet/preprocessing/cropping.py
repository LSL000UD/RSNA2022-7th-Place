...
def get_case_identifier(case):
    ...
    case_identifier = case[0].replace('\\', '/').split("/")[-1].split(".nii.gz")[0][:-5]
    # ---------------------------- Modify from original nnunet --------------------------- #
...
def get_case_identifier_from_npz(case):
    # ---------------------------- Modify from original nnunet --------------------------- #
    case_identifier = case.replace('\\', '/').split("/")[-1][:-4]
    # ---------------------------- Modify from original nnunet --------------------------- #
    return case_identifier
...
def crop_to_nonzero(data, seg=None, nonzero_label=-1):
    ...
    # ---------------------------- Modify from original nnunet --------------------------- #
    if os.environ.get('nnUnet_do_pre_processing') == 'NO':
        bbox = [[0, k] for k in data.shape[1:]]
    else:
...
def get_patient_identifiers_from_cropped_files(folder):
    # ---------------------------- Modify from original nnunet --------------------------- #
    return [i.replace('\\', '/').split("/")[-1][:-4] for i in subfiles(folder, join=True, suffix=".npz")]
# ---------------------------- Modify from original nnunet --------------------------- #
