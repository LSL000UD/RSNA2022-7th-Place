...
class Compose(AbstractTransform):
    ...
    def __init__(self, transforms):
        ...
        if os.environ.get('nnUNet_no_aug_for_all_channel') == 'YES':
    ...
    def __call__(self, **data_dict):
        for t in self.transforms:
            # ---------------------------- Modify from original nnunet --------------------------- #
            if os.environ.get('nnUNet_no_aug_for_all_channel') == 'YES':
                warnings.warn('==> nnUNet_no_aug_for_all_channel !!!!!!!!!!!')
        ...
        # import SimpleITK
        #
        # SimpleITK.WriteImage(SimpleITK.GetImageFromArray(data_dict['data'][0, 1]), '/data/liushuolin/Cache/seg.nii.gz')
        # SimpleITK.WriteImage(SimpleITK.GetImageFromArray(data_dict['data'][0, 0]),
        #                      '/data/liushuolin/Cache/image.nii.gz')
        # SimpleITK.WriteImage(SimpleITK.GetImageFromArray(data_dict['target'][0][0, 0]),
        #                      '/data/liushuolin/Cache/gt.nii.gz')
        return data_dict
